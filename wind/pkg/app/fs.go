...
func (h *fsHandler) handleRequest(c context.Context, ctx *RequestContext) {
	...
	r, err := ff.NewReader()
	if err != nil {
		hlog.SystemLogger().Errorf("无法获取文件阅读器，路径=%q，错误=%s", path, err)
		ctx.AbortWithMsg("内部服务器错误", consts.StatusInternalServerError)
		return
	}
	...
	if !ctx.IsHead() {
		...
		if rc, ok := r.(io.Closer); ok {
			if err := rc.Close(); err != nil {
				hlog.SystemLogger().Errorf("无法关闭文件阅读器：错误=%s", err)
				ctx.AbortWithMsg("内部服务器错误", consts.StatusInternalServerError)
				return
			}
		}
...
// ServeFile 将指定路径的文件内容压缩后写入 HTTP 响应。
//
...
// ServeFileUncompressed 将指定路径的文件内容写入 HTTP 响应。
//
// 若指定路径为目录则返回目录的内容。
//
