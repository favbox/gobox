...
// BodyWriter 返回用于填充响应主体的编写器。
// 如果在 RequestHandler 内部使用，则从 RequestHandler 返回后不得使用返回的编写器。
...
// RemoteAddr 返回远程网络地址。该地址并发共享，勿改。
