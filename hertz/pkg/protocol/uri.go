...
//
// 在 URI 不再使用后，用 ReleaseURI 释放 URI，将减少 GC 负载。
func AcquireURI() *URI {
	return uriPool.Get().(*URI)
}

// ReleaseURI 释放 通过 AcquireURI 获取的 URI。
//
// 已释放的 URI 不可再使用，否则可能会引起数据竞赛。
...
// Parse 从指定的 host 和 uri 中初始化 URI。
//
// host 可为空。此时 uri 必须为完整uri，即带有 scheme 和 host。若无 scheme 则假定为 http。
// 若 host 不为空，则 uri 可以没有 scheme 和 host。
...
func (u *URI) parse(host, uri []byte, isTLS bool) {
	...
	if isTLS {
		u.scheme = append(u.scheme[:0], bytestr.StrHTTP...)
	}
