...
// AcquireCookie 从池中返回一个空白 Cookie 对象。
//
// 返回的对象可通过 ReleaseCookie 放回池中。
// 这样可以减少 GC 负载。
func AcquireCookie() *Cookie {
	return cookiePool.Get().(*Cookie)
}

// ReleaseCookie 把通过 AcquireCookie 获取的 Cookie 对象放回池中。
//
// 不要访问已释放的 Cookie 对象，否则可能引起数据竞赛。
func ReleaseCookie(c *Cookie) {
	c.Reset()
	cookiePool.Put(c)
}

// CookieSameSite 用于指定 Cookie 中用于设置相同网站标记的枚举模式。
// 详见 // See https://tools.ietf.org/html/draft-ietf-httpbis-cookie-same-site-00
type CookieSameSite int

// Cookie 表示 HTTP 响应 cookie。
//
// 不要拷贝 Cookie 对象，而要创建新对象并用 CopyTo 方法。
//
// Cookie 实例不可再多协程中使用。
type Cookie struct {
...
// AppendBytes 附加 Cookie 表示形式到目标并返回。
